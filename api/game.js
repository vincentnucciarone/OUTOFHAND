...
if (!redisUrl || !redisToken) {
  throw new Error(
    'Redis configuration missing on this Vercel deployment. Check KV_REST_API_URL and KV_REST_API_TOKEN.'
  );
}
...
`;
  ...
  try {
    ...
    if (
      ...
      throw new Error('Bad scenario JSON');
    }
...
async function actionMaybeJudge(
  ...
  return await withLock(
    ...
    async () => {
      ...
      for (
        const verdict of verdicts
      ) {
        const player =
          lobby.players.find(
            (player) =>
              player.id === verdict.playerId
          );
